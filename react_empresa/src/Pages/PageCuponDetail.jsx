...
function PageCuponDetail() {
  ...
  useEffect(() => {
    ...
    const obtenerPromociones = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1/proyecto_2_lenguajes/api_php/services/cuponservice.php?descuentosCupon=${params.id}`);
  ...
  const handleButtonClick = async (id) => {
    try {
      const response = await axios.get(`http://127.0.0.1/proyecto_2_lenguajes/api_php/services/cuponservice.php?stateCupon=${id}`);
  ...
  const handlePromocion = async (id) => {
    try {
      const response = await axios.get(`http://127.0.0.1/proyecto_2_lenguajes/api_php/services/cuponservice.php?stateDescuento=${id}`);
      toast.success(response.data);
      setChange(!change);
    } catch (error) {
